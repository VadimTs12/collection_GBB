...
const Language = () => {
    ...
    return (
        <div className="items-center relative h-7 items-center  tracking-wider duration-300 uppercase text-2xs font-500
                        padding-huge  items-center p-4 hover:bg-opacity-70 rounded  flex  flex-row mt-2 lg:mt-0 ">
            <button type="button"
                    onClick={changeLanguage}
                    className=" px-2 py-1 rounded text-black dark:text-white duration-300 bg-gray-300 dark:bg-neutral-800 whitespace-nowrap">
                {lang.languageBtn}
            </button>
            {languageVisible && <div className="language__block absolute z-10  w-25 mt-2 origin-top-right bg-white divide-y
                             rounded-md shadow-lg  top-10 right-2 focus:outline-none">
